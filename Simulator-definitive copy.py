...
class Server(Node, Machine):
    ...
    def receive(self, m):
        if m.event == "end":  # end of service
            ...
            if self.blocked:
                ...
                self.start(job)
                if self.In.blocked and self.queuenotfull:
                    #print('unblock')
                    self.sendunblock()
            else:
                self.busyServers -= 1
            # self.departureStats()
        elif m.event == 'job': # receive new job
            job = m.job
            self.jobsReceived += 1
            job.setArrivalTime(now())
            job.setServiceTime(self.serviceTimeDistribution.rvs())
    ...
    def sendunblock(self):
        if bool(self.observers):
    ...
    def nextqueue(self):
        if len(self.out.queue) >= (self.out.maxQueue):
            return True
        else:
            return False
    ...
    def update(self, arg):
        if "Block" in arg:
            ...
            self.blocked=True
        elif "Unblock" in arg and self.blocked:
            #print("{} {}".format(self, arg))
            self.blocked=False
        else:
            pass
...
class Fifo(Server):
    def __init__(self, name, maxQueue = 10):
...
if __name__ == "__main__":
    ...
    server1 = Fifo('server1', maxQueue=5)
...
print(len(server2.queue))
print(len(server3.queue))